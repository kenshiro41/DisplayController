...
trait Monitors {
    fn get_value(&self, code: u8) -> u16;
}

impl Monitors for Monitor {
    fn get_value(&self, code: u8) -> u16 {
        match self.winapi_get_vcp_feature_and_vcp_feature_reply(code) {
            Ok((_ty, current, _max)) => (current & 0xFF) as u16,
            Err(err) => {
...
pub fn get_displays() -> Vec<Display> {
    ...
    for m in Monitor::enumerate().unwrap() {
...
fn set_value(code: u8, id: u32, value: u16) {
    for m in Monitor::enumerate().unwrap() {
        ...
        if m_id == id {
            let res = m.winapi_set_vcp_feature(code, value as u32);
            if res.is_err() {
                println!("err: {:?}", res.err());
            } else {
